...
export const BASE_URL = URLS.koda.estuary
...
export const pinJson = async (object: Record<string, any>, name: string) => {
  try {
    const { status, data } = await api.post<EstuaryApiResponse>(
      `pinJson/${name}`,
      object
    )
    ...
    if (status < 400) {
      ...
      throw new Error(
        `[PINNING] Unable to PIN JSON for reasons ${status} ${data}`
      )
    }
  } catch (e) {
    console.warn(e)
...
export const pinFileToIPFS = async (
  ...
  try {
    const { status, data } = await Axios.post<EstuaryApiResponse>(
    ...
    if (status < 400) {
      ...
      throw new Error(
        `[PINNING] Unable to PIN Image for reasons ${status} ${data}`
      )
    }
  } catch (e) {
    console.warn(e)
