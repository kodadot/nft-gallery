...
// @ts-nocheck
// follow this code https://github.com/oruga-ui/oruga/blob/develop/packages/oruga-next/src/components/notification/NotificationNotice.vue
import {
  type Component,
  type PropType,
  computed,
...
import { baseComponentProps } from '@oruga-ui/oruga-next/src/utils/SharedProps'
import { getOption } from '@oruga-ui/oruga-next/src/utils/config'
import { u as useComputedClass } from '@oruga-ui/oruga-next/dist/esm/useComputedClass-T7fME1R-.mjs'
import { u as useProgrammaticComponent } from '@oruga-ui/oruga-next/dist/esm/useProgrammatic-9W9wxslI.mjs'

import type { NotifcationProps } from '@oruga-ui/oruga-next/src/components/notification/types'
import type {
  ComponentClass,
  ProgrammaticInstance,
  PropBind,
} from '@oruga-ui/oruga-next/src/types'
...
function setAutoClose(): void {
  if (!props.infinite) {
    ...
    timer.value = setTimeout(() => {
      if (isActive.value) handleClose({ action: 'close', method: 'timeout' })
    }, props.duration)
  }
}
...
const noticeCustomContainerClasses = computed(() => [
  useComputedClass('noticeCustomContainerClass', 'o-notices__custom-container'),
])
