...
export const buildDescription = (options: Options): string => {
  const { gender, framing, art, style, having, wearing, lighting, inspiredBy } =
    options
  let base = `Hello 👋,\n I am anime ${gender} generated by artificial intelligence called Stable Diffusion.\n
  We met at Polkadot Metaverse Championship in Budapest, Hungary 🇭🇺.\n
  I would like to say something about this picture:\n\n
  ...
  if (lighting) {
    base += `Do you like the style of this picture? I think that ${lighting} lighting gives it its own vibe.\n`
  }

  if (inspiredBy) {
    base += `Wanna know a secret? Art was inspired by artist ${inspiredBy}.\n`
  }
