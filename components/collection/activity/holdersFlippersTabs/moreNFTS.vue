...
import { NFTExcludingEvents } from '@/components/collection/utils/useCollectionDetails'
...
const processNFTImages = async () => {
  if (props.nfts) {
    const promises = props.nfts.map(async (nft) => {
      let avatar
      if (nft?.meta?.image) {
      ...
      const nftIndex = nfts.value?.findIndex(({ id }) => id === nft.id)
      nfts.value[nftIndex].avatar = avatar
    })
