...
import { getStashIds, getOwnStashes, IsInKeyring, getAvailableSlashes, extractState, State, getAccounts, getStakerState } from './stashInfo'
...
export default class Actions extends Mixins(SubscribeMixin) {
  ...
  private stateCallBack([accounts, validators]: [any, any]) {
    ...
    const queried = ownStashes.reduce((queried: Queried, [stashId, isOwnStash], index): Queried => ({
      ...queried,
