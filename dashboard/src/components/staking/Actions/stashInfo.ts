...
export function extractState(ownStashes?: StakerState[]): State {
  ...
  return {
    bondedTotal: ownStashes.reduce(
      (total: BN, { stakingLedger }) =>
        stakingLedger ? total.add(stakingLedger.total.unwrap()) : total,
      BN_ZERO
    ),
...
export function getStakerState(
  stashId: string,
  allAccounts: string[],
  allStashes: string[] | undefined,
  [
    isOwnStash,
    {
  ...
  const nextConcat = u8aConcat(
    ...nextSessionIds.map((id: any): Uint8Array => id.toU8a())
  );
  const currConcat = u8aConcat(
    ...sessionIds.map((id: any): Uint8Array => id.toU8a())
  );
  ...
  return {
    controllerId,
    destination:
      (rewardDestination && rewardDestination.toString().toLowerCase()) || '',
