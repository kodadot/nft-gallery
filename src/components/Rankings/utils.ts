...
const sumFn = (acc: number, val: SimpleRankingsNFT) => {
...
const uniqueFn = (acc: Set<string>, val: SimpleRankingsNFT) =>
  acc.add(val.metadata);
const soldFn = (acc: number, val: SimpleRankingsNFT) =>
  val.issuer !== val.currentOwner ? acc + 1 : acc;
const onlyOwned = ({ issuer, currentOwner }: SimpleRankingsNFT) =>
  issuer === currentOwner;
